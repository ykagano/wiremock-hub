...
export const wiremockInstanceApi = {
  ...
  async get(id: string): Promise<WiremockInstance> {
    const response = await apiClient.get<ApiResponse<WiremockInstance>>(
      `/wiremock-instances/${id}`
    );
    return response.data.data!;
  },

  async create(input: CreateWiremockInstanceInput): Promise<WiremockInstance> {
    const response = await apiClient.post<ApiResponse<WiremockInstance>>(
      '/wiremock-instances',
      input
    );
    return response.data.data!;
  ...
  async update(id: string, input: UpdateWiremockInstanceInput): Promise<WiremockInstance> {
    const response = await apiClient.put<ApiResponse<WiremockInstance>>(
      `/wiremock-instances/${id}`,
      input
    );
    return response.data.data!;
  ...
  async getMappings(id: string): Promise<MappingsResponse> {
    const response = await apiClient.get<ApiResponse<MappingsResponse>>(
      `/wiremock-instances/${id}/mappings`
    );
    return response.data.data!;
  },
...
export const stubApi = {
  ...
  async testStub(id: string, overrides?: StubTestRequest): Promise<StubTestResponse> {
    const response = await apiClient.post<ApiResponse<StubTestResponse>>(
      `/stubs/${id}/test`,
      overrides || {}
    );
    return response.data.data!;
