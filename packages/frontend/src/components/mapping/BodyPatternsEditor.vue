<template>
  <div class="body-patterns-editor">
    <div v-for="(pattern, index) in patterns" :key="index" class="pattern-row">
      <el-card>
        <el-form
        ...
        >
          ...
          <el-form-item :label="t('labels.value')">
            <el-input v-model="pattern.value" type="textarea" :rows="4" @input="updateValue" />
          </el-form-item>

          <el-button type="danger" size="small" @click="removePattern(index)">
            {{ t('common.delete') }}
          </el-button>
        </el-form>
      </el-card>
    ...
    <el-button type="primary" plain size="small" @click="addPattern" style="margin-top: 8px">
...
function updateValue() {
  const result = patterns.value.filter((p) => p.value).map((p) => ({ [p.type]: p.value }));
