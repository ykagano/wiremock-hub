...
test.describe('Request Log', () => {
  ...
  test('should display request log, detail and import as stub', async ({ page, request }) => {
    ...
    await expect(page.getByText(/作成しました|created/i).first()).toBeVisible({
      timeout: 5000
    });
    ...
    await expect(page.locator('code', { hasText: '/api/request-log-test' }).first()).toBeVisible({
      timeout: 10000
    });

    // Verify unmatched request log entry is displayed (in All tab)
    await expect(page.locator('code', { hasText: '/api/unmatched-test' }).first()).toBeVisible({
      timeout: 10000
    });
    ...
    const matchedRowInTab = page
      .locator('.el-table__row:visible', { hasText: '/api/request-log-test' })
      .first();
    ...
    await expect(
      page.locator('.body-content', { hasText: 'Request log test response' })
    ...
    const unmatchedRowInTab = page
      .locator('.el-table__row:visible', { hasText: '/api/unmatched-test' })
      .first();
    ...
    await expect(
      page.locator('.el-dialog', { hasText: /スタブとしてインポート|Import as Stub/ })
    ).toBeVisible();
    ...
    // Wait for success message
    await expect(page.getByText(/インポート|import|成功|success/i).first()).toBeVisible({
      timeout: 5000
    });
  ...
  test('should filter requests by URL, method, status code and reset', async ({
    ...
    await page.getByRole('button', { name: /保存|Save/ }).click();
    await expect(page.getByText(/作成しました|created/i).first()).toBeVisible({ timeout: 5000 });
    ...
    const statusToInput = page
      .locator('.el-form-item', { hasText: /ステータス.*まで|Status.*To/i })
      .locator('.el-input-number input');
  ...
  test('should paginate request log when there are many requests', async ({ page, request }) => {
    ...
    await expect(page.locator('code', { hasText: '/api/pagination-test-' }).first()).toBeVisible({
      timeout: 5000
    });
