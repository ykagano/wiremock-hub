...
test.describe('Registered Stubs', () => {
  ...
  test('should display registered stubs after sync', async ({ page }) => {
    ...
    await page
      ...
      .fill('Registered Stubs Instance');
    await page.locator('.el-dialog').getByLabel(/URL/).fill(WIREMOCK_1_URL);
    await page
      .locator('.el-dialog')
      .getByRole('button', { name: /保存|Save/ })
      .click();
    await expect(page.locator('.el-card', { hasText: 'Registered Stubs Instance' })).toBeVisible({
      timeout: 10000
    });
    ...
    await page
      ...
      .click();
    await expect(page.getByText(/同期完了|synced|Sync completed/i).first()).toBeVisible({
      timeout: 15000
    });
    ...
    await page
      .locator('.el-aside')
      .getByText(/登録済みスタブ|Registered Stubs/)
      .click();
    await expect(
      page.getByRole('heading', { name: /登録済みスタブ|Registered Stubs/ })
  ...
  test('should show empty state when no mappings', async ({ page, request }) => {
    ...
    await page
      ...
      .getByRole('button', { name: /保存|Save/ })
      .click();
    await expect(page.locator('.el-card', { hasText: 'Empty Instance' })).toBeVisible({
      timeout: 10000
    });
    ...
    await expect(
      page.locator('.el-empty', {
        hasText: /マッピングが登録されていません|No mappings registered/
      })
    ).toBeVisible({ timeout: 10000 });
  ...
  test('should expand row to show mapping JSON', async ({ page }) => {
    ...
    await page
      .locator('.el-dialog')
      .getByLabel(/インスタンス名|Name/)
      .fill('Expand Test Instance');
    await page.locator('.el-dialog').getByLabel(/URL/).fill(WIREMOCK_1_URL);
    await page
      .locator('.el-dialog')
      .getByRole('button', { name: /保存|Save/ })
      .click();
    await expect(page.locator('.el-card', { hasText: 'Expand Test Instance' })).toBeVisible({
      timeout: 10000
    });
  ...
  test('should delete individual mapping from instance', async ({ page }) => {
    ...
    await page
      .locator('.el-dialog')
      .getByLabel(/インスタンス名|Name/)
      .fill('Delete Test Instance');
    await page.locator('.el-dialog').getByLabel(/URL/).fill(WIREMOCK_1_URL);
    await page
      .locator('.el-dialog')
      .getByRole('button', { name: /保存|Save/ })
      .click();
    await expect(page.locator('.el-card', { hasText: 'Delete Test Instance' })).toBeVisible({
      timeout: 10000
    });
    ...
    await expect(page.locator('.el-table__row', { hasText: testUrl })).not.toBeVisible({
      timeout: 10000
    });
  ...
  test('should delete all mappings from instance', async ({ page }) => {
    ...
    await page
      .locator('.el-dialog')
      .getByLabel(/インスタンス名|Name/)
      .fill('Delete All Instance');
    await page.locator('.el-dialog').getByLabel(/URL/).fill(WIREMOCK_2_URL);
    await page
      .locator('.el-dialog')
      .getByRole('button', { name: /保存|Save/ })
      .click();
    await expect(page.locator('.el-card', { hasText: 'Delete All Instance' })).toBeVisible({
      timeout: 10000
    });
    ...
    // Wait for the table to load
    await expect(page.locator('.el-table__row', { hasText: testUrl1 }).first()).toBeVisible({
      timeout: 10000
    });
  ...
  test('should refresh mappings on button click', async ({ page }) => {
    ...
    await page
      .locator('.el-dialog')
      .getByLabel(/インスタンス名|Name/)
      .fill('Refresh Test Instance');
    await page.locator('.el-dialog').getByLabel(/URL/).fill(WIREMOCK_1_URL);
    await page
      .locator('.el-dialog')
      .getByRole('button', { name: /保存|Save/ })
      .click();
    await expect(page.locator('.el-card', { hasText: 'Refresh Test Instance' })).toBeVisible({
      timeout: 10000
    });
    ...
    const responsePromise = page.waitForResponse(
      (resp) => resp.url().includes('/api/wiremock-instances/') && resp.url().includes('/mappings')
    );
